...
class Processor():
    def __init__(self, minFreq = 61.735, nBPS = 5, nPitch = 50, transRange = 2, transSelf = 0.99, yinTrust = 0.5):
...
class MonoPitchHMM(SparseHMM.Observer):
    def __init__(self, minFreq = 61.735, nBPS = 5, nPitch = 50, transRange = 2, transSelf = 0.99, yinTrust = 0.5): # BPS: bin per semitone
    ...
    def calcObsProb(self, input): # input: [state(freq in Herz), prob]
        ...
        if(probYinPitched > 0.0):
            out[:self.nPitch] *= probReallyPitched / probYinPitched
        out[self.nPitch:] = (1.0 - probReallyPitched) / self.nPitch
